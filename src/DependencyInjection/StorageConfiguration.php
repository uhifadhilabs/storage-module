...
final class StorageConfiguration
{
    ...
    public static function define(NodeDefinition|ArrayNodeDefinition $root): void
    {
        ...
        $root
            ...
            ->children()
                ->arrayNode('files')
                    ->info('The Files hub — the cross-module screen at /files. It needs SecurityBundle, Twig and the host’s widget framework; where any of those is absent the screens are simply not registered.')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->booleanNode('enabled')
                        ...
                        ->scalarNode('settings_permission')
                            ->info('What “Where files go” asks for. Seeing where files are kept is seeing something about every file at once, so it rides on the deployment’s administrator permission — set it to the host’s own Modules permission where there is one.')
